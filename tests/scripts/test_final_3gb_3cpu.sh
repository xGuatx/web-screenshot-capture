...
# Test Final - Configuration Optimale 3GB/3CPU/3/3
...
echo "Configuration: 3GB RAM / 3 CPU / 3 browsers / 3 prewarm"
echo "=========================================="
echo ""
...
# Configurer 3GB RAM / 3 CPU
echo "[2/6] Configuration VM: 3GB RAM + 3 CPU cores"
VBoxManage modifyvm "$VM_NAME" --memory 3072 --cpus 3
...
for i in {1..30}; do
    if ssh -o ConnectTimeout=2 -o StrictHostKeyChecking=no ${VM_USER}@${VM_IP} "echo 'SSH OK'" &>/dev/null; then
        echo "SSH disponible apres $((i*2))s"
        break
    fi
    sleep 2
done
...
ssh ${VM_USER}@${VM_IP} "docker exec shoturl-v3 printenv | grep -E 'MAX_CONCURRENT_BROWSERS|PREWARM_COUNT|BROWSER_TIMEOUT|PAGE_LOAD_TIMEOUT'"
